...
def resource():
    ...
    if token.split(" ")[1].startswith("testing:") and token.split(" ")[1] in cfg.db_acs:
        ...
        toLog = toLog + " (AAD verify time " + str(elapsed) + "s) "

        if response.status_code == 401:
            toLog = toLog + " -> 401:Bad Token"
            closeOffLog(toLog)
            return Response("401 UNAUTHORIZED: Azure AD Access token does not correspond to any user, was tampered with, has expired or is otherwise invalid.", status=401, mimetype='text/plain')
        ...
        if userPrincipalName.lower().endswith("mcpsmd.org") == False:
            ...
            closeOffLog(toLog)
            return Response("401 UNAUTHORIZED: The Azure AD user behind this token is not an MCPSMD user.", status=401, mimetype='text/plain')
        else:
